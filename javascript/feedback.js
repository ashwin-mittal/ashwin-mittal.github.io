...
document.addEventListener('DOMContentLoaded', () => {
    if (localStorage.getItem('name') !== null) {
        ...
        for (let num = 0; num < prevName.length; num++)
            content.innerHTML += '<tr><th scope="row">' + (num + 1) + '</th><td>' + prevName[num] + '</td><td>' + prevSkill[num] + '</td><td>' + prevLevel[num] + '</td><td>' + prevComment[num] + '</td></tr>';
    }
    num++;
});
function tableMaker() {
    ...
    if (localStorage.getItem('name') === null) {
        ...
        var l = localStorage.getItem('level') + '#' + level.options[level.selectedIndex].text;
